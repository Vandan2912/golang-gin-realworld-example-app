#!/usr/bin/env bash
# Runs the RealWorld API spec test suite (Bruno flavor) against this backend.
# The Bruno collection is generated from the Hurl suite in
# https://github.com/realworld-apps/realworld under specs/api/bruno.
#
# Usage: bash ./scripts/run-bruno-tests.sh
# Requires: go, curl, node/npx (or a globally installed @usebruno/cli)
set -euo pipefail
